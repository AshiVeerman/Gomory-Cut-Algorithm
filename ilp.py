...
def convert_std(A, b, c, n, m):
    ...
    table = var_art(table, art_add, n, m)
...
#print(table)    # is printing the standard form correctly 
...
def remake_tableau(table, art, rem_rows):
    ...
    #print(art)
    l = len(table[:,0])
    l_col_n = (len(table[0,:])-len(rem_rows))
    l_row_n = len(table[:,0]) - len(art)
...
def final_answer(tableau,l,n):
    (tableau,l)=gomory_implementation(tableau,l)
    answer=[0]*n
    for i in range (len(l)):
        if l[i]<=n:
...
# NOTE: Current code is for MINIMISATION, we need code for MAXIMISATION

# has been tested for one problem, we now test for a more complex problem 
